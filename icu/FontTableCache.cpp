...
{
    fTableCache = (FontTableCacheEntry *) malloc(sizeof(FontTableCacheEntry) * fTableCacheSize);
...
void FontTableCache::add(LETag tableTag, const void *table)
{
    if (fTableCacheCurr >= fTableCacheSize) {
        ...
        fTableCache = (FontTableCacheEntry *) realloc(fTableCache, newSize * sizeof(fTableCache[0]));
